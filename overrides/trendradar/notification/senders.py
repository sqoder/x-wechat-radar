...
def _iter_rss_title_entries(rss_blocks: Optional[list]):
    """遍历 RSS 统计结构中的标题条目"""
    if not rss_blocks:
        return

    for block in rss_blocks:
...
def _extract_summary_url(summary: str, label: str) -> str:
    """从 summary 中提取 `标签: URL` 格式的媒体链接"""
    if not summary:
        return ""
...
def _collect_wework_media_entries(
    rss_items: Optional[list], rss_new_items: Optional[list], max_items: int
) -> List[Dict[str, str]]:
    """从 RSS 条目中收集包含图片/视频的动态（优先新增）"""
    media_entries: List[Dict[str, str]] = []
    seen_keys = set()

    # 先用新增，再补历史，优先保证看到最新动态
    source_blocks = [rss_new_items, rss_items]
    for blocks in source_blocks:
        for entry in _iter_rss_title_entries(blocks):
            ...
            title = str(entry.get("title", "") or "").strip() or "X 动态"
            feed_name = str(entry.get("source_name", "") or "").strip()
            published_at = str(entry.get("published_at", "") or "").strip()

            dedupe_key = post_url or f"{image_url}|{video_url}"
...
def _send_wework_image(
    ...
    try:
        ...
        if response.status_code != 200 or not response.content:
            print(f"{log_prefix}图片下载失败，状态码: {response.status_code}，url: {image_url}")
            return False
        ...
        if _post_wework_payload(
            webhook_url=webhook_url, payload=payload, headers=headers, proxies=proxies
        ):
            return True

        print(f"{log_prefix}图片发送失败，url: {image_url}")
        return False
    except Exception as e:
        print(f"{log_prefix}图片发送异常: {e}")
        return False


def _send_wework_video_card(
    webhook_url: str,
    ...
    """发送视频卡片（企业微信不支持直接上传视频，使用 news 卡片跳转）"""
    title = media_entry.get("title", "X 视频动态")
    feed_name = media_entry.get("feed_name", "")
    published_at = media_entry.get("published_at", "")
    post_url = media_entry.get("post_url", "")
    video_url = media_entry.get("video_url", "")
    image_url = media_entry.get("image_url", "")

    description_parts = ["点击卡片查看原帖视频"]
    if feed_name:
        description_parts.append(f"来源: {feed_name}")
    if published_at:
        description_parts.append(f"时间: {published_at}")
    description = " | ".join(description_parts)

    article = {
        "title": f"🎬 {title}"[:128],
        "description": description[:512],
        "url": (post_url or video_url)[:2048],
    }
    if image_url:
        article["picurl"] = image_url[:2048]
    ...
    # card 失败时，降级为 text，仍然把视频链接发出去
    fallback_content = (
        f"🎬 {title}\n"
        f"原帖: {post_url or '无'}\n"
        f"视频直链: {video_url or '无'}"
    )
    fallback_payload = {"msgtype": "text", "text": {"content": fallback_content}}
    if _post_wework_payload(
    ...
    print(f"{log_prefix}视频卡片发送失败，post_url: {post_url}")
    return False


def _send_wework_rss_media_followups(
    webhook_url: str,
    ...
    """补发 RSS 媒体：图片直发 + 视频卡片"""
    media_entries = _collect_wework_media_entries(
        rss_items=rss_items,
        rss_new_items=rss_new_items,
        max_items=WEWORK_MEDIA_MAX_ITEMS,
    )
    if not media_entries:
        print(f"{log_prefix}本次无可补发的图片/视频")
        return

    print(f"{log_prefix}检测到 {len(media_entries)} 条媒体动态，开始补发图片/视频")
    ...
    print(f"{log_prefix}媒体补发完成，成功发送 {sent_count} 条")
...
def send_to_wework(
    ...
    *,
    ...
    _send_wework_rss_media_followups(
        webhook_url=webhook_url,
        rss_items=rss_items,
        rss_new_items=rss_new_items,
        headers=headers,
